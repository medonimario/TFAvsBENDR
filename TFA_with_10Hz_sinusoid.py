...
def epochsTFR_to_df(epochsTFR_p1_uncoupled, epochsTFR_p1_coupled, epochsTFR_p2_uncoupled, epochsTFR_p2_coupled, _19_channels=False):
    ...
    #print(flattened_uncoupled.shape)
...
if __name__ == "__main__":
    ...
    for pair_nr in pairs:
        ...
        p1_uncoupled_epochs = add_sinusoid_to_epochs(p1_uncoupled_epochs, frequency=10.0, strength_percent=0.5, channels='all')
        p2_uncoupled_epochs = add_sinusoid_to_epochs(p2_uncoupled_epochs, frequency=10.0, strength_percent=0.5, channels='all')
        ...
        # Save dataframe
        if _19_channels:
            df.to_csv(f"C:/1_University/Thesis/Scripts/TFA_10Hz/pair{pair_nr}_df.csv", index=False)
        else:
            df.to_csv(f"C:/1_University/Thesis/Scripts/TFA_10Hz_64/pair{pair_nr}_df.csv", index=False)